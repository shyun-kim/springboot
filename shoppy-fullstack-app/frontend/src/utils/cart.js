...
export function cartItemsCheck(prevItems, cartItem) {
    ...
    if(existItem) { //존재하면 map으로 순회 하면서 pid, size가 동일한 item에 qty+1증가
        return prevItems.map((item) => //map는 새로운 배열 반환
        item.pid === cartItem.pid && item.size === cartItem.size 
            ? { ...item, qty: item.qty+1 }
            :item
        );
        ...
        return [...prevItems, {...cartItem, cid:cid}]; // 존재하지 않으면 새로운 아이템 추가
    }
}
